def main():
    ...
    physical_frame_table = get_physical_frame_table(
        page_mapping, physical_frame_table, virtual_page_table, sorted_mapping[-1], word_size)
...
def set_virtual_page_table(virtual_page_table=[], size=0, word_size=0):
    ...
    for offset in range(0, size):
        for elem in range(0, word_size):
            temp.append(
                input("Enter value for offset #{}[{}]: ".format(offset, elem)))
...
def get_physical_frame_table(page_mapping=[], physical_frame_table=[], virtual_page_table=[], upperbound=0, word_size=0):
    for frame_number in range(0, upperbound + 1):
        physical_frame_table.append([
            ...
            virtual_page_table[page_mapping.index(
                frame_number)] if frame_number in page_mapping else None
        ])
